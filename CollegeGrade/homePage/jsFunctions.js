var main = function (user,cls) {
   ...
   graphGPAcum($(".graph").css("width"),
	       $(".graph").css("padding"),
	       $(".graph").css("height"));
   $('svg').after('<button type="button" class="btn btn-secondary gpaSecond">GPA per semester</button>');
   $('svg').after('<button type="button" class="btn btn-secondary gpaFirst">GPA current Semester </button>');
   }
   else {
   ...
   $('.mask').hide();
   $(".sem").click(function () {
    $(".slideInMenu").animate({"margin-left": "0px"},"slow");
    $(this).hide();
    $(".mask").fadeIn(500);
    $(".mask").css("z-index","3");
   ...
   $(".icon-close").click( function () {
    $(".slideInMenu").animate({"margin-left": "-450px"});
    $(".btn").show();
    $(".mask").fadeOut(500);
    $(".mask").css("z-index","0");
   });
...
var graphGrade = function (w,p,h) {
 ...
 svg.append("g").attr("class","axis").attr("transform", "translate(0," + (h - 2*p) + ")").call(xaxis); 
 svg.append("g").attr("class","axis").attr("transform", "translate(" +p+ ",0)").call(yaxis);
 var rec = svg.append("rect").attr("x",p).attr("y",p).attr("height",h/10).attr("width",w/10);
 rec.append("circle").attr("fill","black").attr("cx",4*p).attr("cy",4*p).attr("r",0,5);
 rec.append("text").text("Your score").attr('x', 4.5*p).attr('y', 4*p);
 rec.append("circle").attr("fill","grey").attr("cx",6*p).attr("cy",6*p).attr("r",0.5);
 rec.append("text").text("Average score").attr('x', 6.5*p).attr('y', 6*p);
 }
